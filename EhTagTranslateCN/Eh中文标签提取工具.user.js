...
// @version      1.0
// @description  从 ExHentai/E-Hentai 提取标签，支持中文翻译、自定义翻译和远程数据库
...
(async function() {
    ...
    // ==================== 数据库配置（来自脚本二） ====================
    const DB_CACHE_KEY = 'ehTagDatabaseCache';
    const DB_CACHE_EXPIRY_KEY = 'ehTagDatabaseExpiry';
    const DB_CACHE_DURATION = 24 * 60 * 60 * 1000; // 24小时
    ...
    async function initializeDatabase() {
        const now = Date.now();
        const expiry = GM_getValue(DB_CACHE_EXPIRY_KEY, 0);

        if (expiry > now) {
            const cached = GM_getValue(DB_CACHE_KEY, null);
            if (cached) {
                dbCategoryMap = cached.categoryMap || {};
                dbTagMap = cached.tagMap || {};
                console.log('使用本地缓存数据库');
                return;
            }
        }
        ...
        try {
            await fetchDatabase();
            GM_setValue(DB_CACHE_KEY, { categoryMap: dbCategoryMap, tagMap: dbTagMap });
            GM_setValue(DB_CACHE_EXPIRY_KEY, now + DB_CACHE_DURATION);
    ...
    async function fetchDatabase() {
        ...
        categories.forEach(([englishName, chineseName]) => {
            dbCategoryMap[englishName] = chineseName;
        });

        results.forEach(result => {
            if (result) {
                Object.assign(dbTagMap, result);
            }
        });
    }
        ...
        for (const line of lines) {
            if (line.startsWith('|') && !line.includes('---')) {
                ...
                if (cells.length >= 2) {
                    ...
                    if (englishName !== 'English' && chineseName !== 'Chinese' && englishName.length > 0) {
    ...
    function getTranslation(tag, category = null) {
        ...
        // 优先级 1️⃣：自定义翻译（最高）
        if (customTranslations[normalizedTag]) {
            return customTranslations[normalizedTag];
        }

        // 优先级 2️⃣：条件翻译（根据分类选择）
        if (contextualTranslations[normalizedTag] && category) {
            const contextualResult = contextualTranslations[normalizedTag][category];
            if (contextualResult) {
        ...
        if (tagTranslations[normalizedTag]) {
            return tagTranslations[normalizedTag];
        }
    ...
    function clearDatabaseCache() {
        const confirmed = confirm('⚠️ 确定要清除数据库缓存吗？此操作无法撤销。');
        if (!confirmed) return;

        GM_deleteValue(DB_CACHE_KEY);
        GM_deleteValue(DB_CACHE_EXPIRY_KEY);
        dbCategoryMap = {};
        dbTagMap = {};
        alert('✓ 缓存已清除');
    }
    ...
    function formatTags(tagsByCategory) {
        const sortedCategories = categoryOrder.filter(cat => tagsByCategory[cat]);
        const formattedTags = [];

        sortedCategories.forEach(category => {
            const chineseCategoryName = categoryNames[category] || category;
            const tags = tagsByCategory[category];
            const formattedCategoryTags = tags.map(tag => {
                const translation = getTranslation(tag, category);
                return `#${translation}`;
            });

            if (formattedCategoryTags.length > 0) {
                formattedTags.push(`${chineseCategoryName}：${formattedCategoryTags.join('，')}`);
            }
        });
    ...
    function createFloatingButtons() {
        ...
        const buttons = [
            ...
            { text: '➕ 添加翻译', onClick: showAddTranslationDialog, danger: false },
            { text: '📥 导出翻译', onClick: exportTranslations, danger: false },
            { text: '📤 导入翻译', onClick: importTranslations, danger: false },
            { text: '🗑️ 清除翻译', onClick: clearAllTranslations, danger: true },
            { text: '🔄 清除缓存', onClick: clearDatabaseCache, danger: true }
        ];
        ...
        buttons.forEach(btn => {
            ...
            button.style.cssText = `
                padding: 8px 12px;
                background: ${btn.danger ? '#f44336' : '#4CAF50'};
                color: white;
                border: none;
                border-radius: 4px;
                cursor: pointer;
    ...
    function exportTranslations() {
        ...
        // 转换为纯文本格式：每行一条 "英文|中文"
        const lines = Object.entries(customTranslations).map(
            ([english, chinese]) => `${english}|${chinese}`
        );
        const exportText = lines.join('\n');

        GM_setClipboard(exportText);
        alert(`✓ 已导出 ${Object.keys(customTranslations).length} 条翻译到剪贴板\n\n${exportText}`);
    }

    // 导入翻译 - 支持两种格式
    function importTranslations() {
        const input = prompt('请粘贴翻译数据，支持两种格式：\n\n1. 行格式（推荐）：\nbig ass|大屁股\nsmall breasts|贫乳\n\n2. JSON 格式：\n{"big ass":"大屁股","small breasts":"贫乳"}');
        ...
        try {
            ...
            // 否则按行格式解析
                ...
                lines.forEach(line => {
                    ...
                    if (english && chinese) {
                        translations[english.toLowerCase()] = chinese;
                    }
                });

                if (Object.keys(translations).length === 0) {
                    throw new Error('未找到有效的翻译数据');
                }
            }
